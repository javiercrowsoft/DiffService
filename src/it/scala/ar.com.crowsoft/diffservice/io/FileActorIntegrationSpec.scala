...
class FileActorIntegrationSpec extends WordSpec
  with Matchers
  ...
  override def createActorSystem() = {
    ...
    ActorSystem("FileActorIntegrationSpecSystem", config)
  }
    ...
    val result = (fileActor ? SaveFile(fileId, FileData(name, data), fileSide)).mapTo[FileSaveResult]
        ...
        result onComplete {
          case Success(saveResult) =>
            ...
            new java.io.File(saveResult.path).exists() should be(true)
          case Failure(e) =>
            fail()
        }
  }
